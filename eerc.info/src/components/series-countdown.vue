<template>
	<div>
		...
		<div v-else class="inprogress">
			<p class="title">{{currentEventName}}</p>
...
export default {
	...
	computed: {
		currentEvent() {
			return this.series.seasons[this.currentSeasonIndex].events[this.currentEventIndex]
		},
		currentSeason() {
			return this.series.seasons[this.currentSeasonIndex]
		},
		currentEventName() {
			return `${this.series.game} - ${this.series.series} - ${this.currentSeason.seasonName} - Round ${this.currentEventIndex + 1} - ${this
				.currentEvent.track ||
	...
	methods: {
		...
		elapsed() {
			...
			this.waitDate.setMinutes(this.waitDate.getMinutes() + this.currentEvent.duration)
