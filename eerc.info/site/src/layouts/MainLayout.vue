  ...
  <q-layout view="lHh Lpr lff">
    <q-header elevated reveal>
      <q-toolbar v-if="!$q.platform.is.mobile && !$q.screen.xs">
        ...
        <q-space />
        <q-img
          src="/logos/grid-finder-partner-logo.png"
          :ratio="1 / 1"
          class="logo q-ma-md"
          style="cursor:pointer;"
          @click="openURL(socials.gridFinder)"
        />
        <q-btn
          icon="mdi-discord"
          @click="openURL(socials.discord)"
          flat
          :label="!$q.screen.sm ? 'Join our Discord!' : ''"
        />
      </q-toolbar>
    ...
    <q-drawer
      v-model="drawer"
      v-if="$q.platform.is.mobile || $q.screen.xs"
      side="right"
      elevated
      content-class="column items-stretch justify-center q-pa-md bg-primary"
    >
      <q-list separator class="self-stretch justify-center full-height column">
        ...
        <q-item class="text-h4 text-bold text-left" clickable @click="openURL(socials.discord)">
          <q-item-section>
            <q-item-label>
              Discord
            </q-item-label>
    ...
    <q-footer>
      ...
      <q-toolbar class="row fill">
        <div
          ...
          :class="$q.platform.is.mobile || $q.screen.xs ? 'justify-center text-center' : 'justify-between'"
        >
      ...
      <q-toolbar class="row fill justify-center">
        ...
        <div class="flagsAttribute text-italic text-caption">
          Icons made by
          <a class="text-bold" target="_blank" href="https://www.freepik.com" title="Freepik">Freepik</a> from
          <a class="text-bold" target="_blank" href="https://www.flaticon.com/" title="Flaticon">www.flaticon.com</a>
        </div>
      </q-toolbar>
      <q-toolbar>
        {{ $q.screen.name }}
...
.page-container {
  background: linear-gradient(rgba(18, 18, 18, 0.65), rgba(18, 18, 18, 0.65)),
    url('/images/background/eerc_background.png') no-repeat fixed, rgb(18, 18, 18);
